   ...
   <form action="{{ url('dash/helpmes/update',$helpme->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row d-flex justify-content-start">
         ...
         <div class=" mb-3 col-md-4 my-1">
            ...
            <input type="text" class="form-control" name="{{'name_'.$locale}}" value="<?php echo $helpme->{'name_'.$locale}; ?>">
         </div>
         <div class=" mb-3 col-md-3 my-1">
            <label for="title">{{__('home.location').'-'.$value}}</label>
            <input type="text" class="form-control" name="{{'address_'.$locale}}" value="<?php echo $helpme->{'address_'.$locale}; ?>">
         </div>
         <div class=" mb-3 col-md-5 my-1">
            <label for="title">{{__('home.problem_subject').'-'.$value}}</label>
            <input type="text" class="form-control" name="{{'problem_title_'.$locale}}" value="<?php echo $helpme->{'problem_title_'.$locale}; ?>">
         </div>
         <div class=" my-1 col">
            <label for="details">{{__('home.problem_details_and').'-'.$value}}</label>
            <textarea class="form-control"  name="{{'problem_details_'.$locale}}" style="height: 100px;"><?php echo $helpme->{'problem_details_'.$locale}; ?></textarea>
         </div>
         {{-- 
